...
void Merge(Node* dummy_head, int x, int y){
    ...
    if(x==y) return;
    int i, j;
    Node* stack_x=NULL; Node* stack_y=NULL;
    Node* cur=dummy_head->next; Node* pre_x = dummy_head;
    ...
    for(i=0; i<stack_y->size; i++){
        data[i] = stack_y->data[i];
    }
    for(j=0; j<stack_x->size; j++, i++){
        data[i] = stack_x->data[j];
    }
    stack_y->data = data;
...
void Cut(Node* dummy_head, int x, int y){
    ...
    for(i=0; i<y; i++){
        data_x[i] = stack_x->data[i];
    }
    for(j=0; i<stack_x->size; j++, i++){
        data_new[j] = stack_x->data[i];
    }
